...
`go-ruby-friendly-id/friendly-id` reimplements Ruby's `friendly-id` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-friendly-id/friendly-id` directly, with no Ruby runtime;
...
Because `friendly-id` is pure Go with cgo disabled, it also compiles to **WebAssembly**
